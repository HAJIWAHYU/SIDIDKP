...
<div class="space-y-8 max-w-2xl">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <h2 class="text-2xl font-bold text-slate-800">Tambah Bidang Baru</h2>
            <p class="text-xs text-slate-500 mt-1">Buat divisi atau unit bidang baru dalam bagan organisasi E-Office.</p>
        </div>
        <div>
    ...
    <div class="bg-white rounded-2xl border border-slate-200/60 p-6 shadow-sm">
        <form action="{{ route('bidang.store') }}" method="POST" class="space-y-6">
            @csrf
            ...
            <div class="space-y-1.5">
                ...
                <input type="text" id="nama_bidang" name="nama_bidang" value="{{ old('nama_bidang') }}" required placeholder="Contoh: Bidang Hubungan Masyarakat" class="w-full bg-white border border-slate-200 rounded-xl px-4 py-2.5 text-xs text-slate-700 focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 transition-all">
                @error('nama_bidang')
